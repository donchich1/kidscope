...
# START_STUDENTS is now dynamically loaded from the JSON file
START_STUDENTS = []  # Placeholder, data will be loaded dynamically
...
def main():
    # Запускаємо потік для періодичного оновлення даних
    threading.Thread(target=load_db_periodically, daemon=True).start()

    # Створюємо та запускаємо Telegram-бот
    app = build_app()
    app.run_polling()
